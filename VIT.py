...
model = ViT(img_dim=200, in_channels=3, patch_dim=10, num_classes=2, dim=512)
model.train()
...
def train_model(epoch, optimizer):
    ...
    for i in range(epoch):
        for j in range(int(training_set_size * 2 / batch_size)):
            ...
            output = softmax_layer(model(batch_input_image))
...
plt.plot(loss_array)
plt.title('Training VIT model on mixed image set')
plt.xlabel('epoch')
plt.ylabel('loss')
plt.show()
